...
$cs_google_settings = get_option('cs_theme_options');
	 ...
	 $ID = email_exists($email);	
      ...
      if ($ID == NULL) { // Register
        ...
        if ($ID == false) { // Real register
          ...
          if (!is_wp_error($ID)) {
            cs_wp_new_user_notification($ID, $random_password);
      ...
      if ($ID) { // Login
        ...
        set_site_transient($user_info->ID.'_cs_google_admin_notice',__('This Google profile is already linked with other account. Linking process failed!', 'EDULMS'), 3600);
...
function cs_google_link_button() {
  ...
  global $cs_google_settings;
  $cs_google_settings['google_login_button'] = 'Google';
  $images_url = edulms::plugin_url().'include/edulms-login/cs-social-login/media/img/';
