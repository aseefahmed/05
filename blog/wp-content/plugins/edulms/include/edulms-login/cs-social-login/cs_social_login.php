...
function cs_social_process_login($is_ajax = false) {
    ...
    switch ($social_login_provider) {
        case 'facebook':
            ...
            $cs_email = isset($fb_json->{ 'email' }) ? $fb_json->{ 'email' } : '';
...
function cs_social_comment_meta($link) {
    ...
    $social_login_comment_via_provider = isset($comment->comment_ID) ? get_comment_meta($comment->comment_ID, 'social_login_comment_via_provider', true): '';
...
function cs_http_get_contents($url) {
    ...
    if (is_wp_error($response)) {
        die(sprintf(__('Something went wrong: %s', 'EDULMS'), $response->get_error_message()));
    } else {
...
function cs_twitter_callback() {
    ...
    if (!class_exists('TwitterOAuth')) {
        require_once 'cs-twitter/twitteroauth.php';
    }
...
function cs_facebook_callback() {
    ...
    $client_id = $cs_theme_option['cs_facebook_app_id'];
    $secret_key = $cs_theme_option['cs_facebook_secret'];
